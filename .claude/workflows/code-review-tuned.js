export const meta = {
  name: "code-review-tuned",
  description: "TUNED-V1 cost-optimized code review — scope ships the diff verbatim, finders run only the angles that apply, Haiku verifies per-file, then a ranked, capped report.",
  whenToUse: "Launched by the /code-review skill at high, xhigh, or max effort. Pass args as \"<level> [target]\" — level is high, xhigh, or max; target is an optional PR number, branch, ref range, path, or free-form review instructions.",
  phases: [
    { title: "Scope", detail: "Pin the diff, capture it verbatim, pick applicable angles", model: "sonnet" },
    { title: "Find", detail: "One finder per applicable correctness angle plus one cleanup finder", model: "sonnet" },
    { title: "Verify", detail: "One Haiku verifier per file — CONFIRMED / PLAUSIBLE / REFUTED", model: "haiku" },
    { title: "Sweep", detail: "Fresh finder hunting only for gaps (xhigh/max)", model: "sonnet" },
    { title: "Synthesize", detail: "Merge duplicates, rank, cap the report", model: "sonnet" },
  ],
}
...
// HOW TO RUN THIS
//
//   Workflow({ scriptPath: ".claude/workflows/code-review-tuned.js",
//              args: "xhigh [optional target]" })
//
// It must be invoked by scriptPath, NOT by name. This runtime resolves
// Workflow({name}) against built-ins only (verified: the registry lists just
// "deep-research" and "code-review"), and a file named code-review.js here
// does NOT shadow the built-in — that was tested and the built-in won. The
// .claude/workflows/ path is for human discoverability, not auto-loading.
// ─────────────────────────────────────────────────────────────────────────

// ─────────────────────────────────────────────────────────────────────────
// Tuned from a measured xhigh run: 1,116,802 tokens / 20 agents / 210 tool
// calls. Where it went: Verify 45%, Find 40%, Sweep 6%, Scope 4%, Synth 4%.
// Three cuts, none of which touch catch rate:
//
//   1. Scope returns the diff TEXT, not just the command. All 20 agents were
//      re-running the same `git diff` and re-Reading the same files (~10 tool
//      calls each). The block is byte-identical across agents, so the prompt
//      cache absorbs most of the added input.
//   2. Verify runs on Haiku at low effort, grouped per FILE (not per
//      file:line). It refuted 0 of 17 candidates while eating 45% of spend —
//      it quotes a line and rules, which does not need a mid-tier model.
//   3. Finders run only the angles Scope says apply. angle-E (wrapper/proxy)
//      cost 60k and returned zero candidates on a React/Next diff with no
//      delegating types.
//
// The sweep stays on Sonnet at full effort on purpose: it looks like an easy
// cut at 67k, but both candidates it surfaced became top-4 findings —
// including one that all six primary finders missed.
// ─────────────────────────────────────────────────────────────────────────
...
const SWEEP_MAX = 8
// Candidates per verifier agent. Files rarely exceed this; when one does it
// splits into extra agents rather than handing a verifier an unbounded list.
const VERIFY_CHUNK = 6
// Above this the verbatim diff stops paying for itself — 20 copies of a huge
// diff costs more than letting each agent read the parts it needs.
const DIFF_CHAR_CAP = 60000
...
// ─── Schemas ───
const SCOPE_SCHEMA = {
  type: "object", required: ["diffCommand", "files", "summary", "diffText", "applicableAngles"],
  properties: {
    ...
    diffText: { type: "string", description: "the VERBATIM stdout of the diff command — full hunks, unabridged, no commentary. Empty string only if the diff exceeds ~60k characters." },
    applicableAngles: { type: "array", items: { enum: ["angle-A", "angle-B", "angle-C", "angle-D", "angle-E"] }, description: "which correctness angles are structurally relevant to THIS diff" },
  },
}
...
// ─── Phase 0: Scope ───
// Low effort: this stage runs git, lists files, and reads CLAUDE.md. The one
// judgment call (which angles apply) is a structural read of the diff.
phase("Scope")
const scope = await agent(
  ...
  "5. Return the diff's VERBATIM stdout in `diffText` — every hunk, unabridged, exactly as git printed it, with no summarizing, eliding, or commentary. Downstream reviewers read the diff from this field instead of re-running the command, so anything you drop here is invisible to the entire review. If the diff exceeds roughly 60,000 characters, return an empty string for `diffText` instead of a partial one.\n" +
  "6. In `applicableAngles`, list which correctness angles are structurally relevant to THIS diff. Judge by what the code actually contains, not by what is conceivable:\n" +
  "   - angle-A (line-by-line scan) — always applicable; always include it.\n" +
  "   - angle-B (removed-behavior audit) — include when the diff deletes or replaces meaningful lines.\n" +
  "   - angle-C (cross-file tracer) — include when the diff changes a function/type used by other files.\n" +
  "   - angle-D (language pitfalls) — include when the diff has logic prone to the language's classic footguns (coercion, closures, dates/timezones, floats, string escaping).\n" +
  "   - angle-E (wrapper/proxy correctness) — include ONLY when the diff adds or modifies a type that wraps/delegates to another (cache, proxy, decorator, adapter). Omit it for ordinary application code.\n" +
  "   Omitting an angle skips a reviewer, so omit only when the angle has genuinely nothing to examine.\n\n" +
  "Return diffCommand exactly as a reviewer should run it. Structured output only.",
  { label: "scope", model: "sonnet", effort: "low", schema: SCOPE_SCHEMA }
)
if (!scope) {
  return { variant: "tuned-v1", error: "Scope agent returned no result — cannot establish the review scope." }
}
if (!scope.files || scope.files.length === 0) {
  return { variant: "tuned-v1", level: LEVEL, target: TARGET || undefined, summary: "No changes found to review.", findings: [], stats: { finders: 0, candidates: 0, verifierAgents: 0, verified: 0 } }
}
log(LEVEL + " review: " + scope.files.length + " changed files")

// Optimization 1 — embed the diff verbatim so no finder/verifier re-runs it.
const diffText = typeof scope.diffText === "string" ? scope.diffText : ""
const diffEmbedded = diffText.length > 0 && diffText.length <= DIFF_CHAR_CAP
if (diffEmbedded) {
  log("diff embedded in prompts (" + diffText.length + " chars) — agents skip re-running it")
} else {
  log("diff NOT embedded (" + (diffText.length === 0 ? "scope returned none" : diffText.length + " chars, over cap") + ") — agents will run the diff command themselves")
}
const DIFF_BLOCK = diffEmbedded
  ? "\n## The diff (verbatim — already run for you)\n" +
    "This is the complete output of the diff command. Read it here rather than\n" +
    "re-running the command. Do still Read the surrounding source files when you\n" +
    "need context the diff does not show (enclosing functions, callers, helpers).\n\n" +
    "```diff\n" + diffText + "\n```\n"
  : ""
...
const FINDER_PROMPT = f => {
  ...
  return "## Code-review finder — " + f.label + "\n\n" + SCOPE_BLOCK + "\n" +
    (isCleanup
      ? "Review the diff above through EACH of the following cleanup lenses:\n\n"
      : "Review the diff above ONLY through the lens of your assigned angle:\n\n") +
    f.text + "\n" +
...
// scope.files so every downstream consumer sees the same path. Longest match
// wins so that when one changed-file path is itself a suffix of another
...
const GROUP_VERIFIER_PROMPT = group =>
  ...
  group.map((c, i) =>
    "[" + i + "] Line " + (c.line != null ? c.line : "(unspecified)") + " — " + c.summary + "\n" +
    "    Failure scenario: " + c.failure_scenario
  ).join("\n") + "\n\n" +
  "Read " + group[0].file + " (and any other file you need) and return one verdict per candidate. " +
  "Judge EACH candidate independently on its own claim — candidates in the same file may describe distinct issues, the same issue, or a mix. " +
  "Reference each by its [i] index.\n\n" +
  VERDICT_LADDER + "\n\n" + VERDICT_LADDER_RECALL + "\n\n" +
  "Structured output only. Evidence must quote or cite the relevant line(s)."

// ─── Optimization 2 — verify per FILE on Haiku at low effort.
// Grouping by file (not file:line) means one agent reads a file once and rules
// on every candidate in it. Grouping is not dedup: every candidate keeps its
// own verdict; the synthesis step merges semantic dupes. A candidate the
// verifier did not render a verdict on (agent died, or it omitted that index)
// is dropped, so unverified candidates never reach the report as fabricated
// PLAUSIBLE. Trade-off: one verifier failure drops every candidate in that
// file chunk, which is why chunks are capped at VERIFY_CHUNK.
...
async function verifyGroups(candidates) {
  ...
  for (const list of Object.values(byFile)) {
    for (let i = 0; i < list.length; i += VERIFY_CHUNK) groups.push(list.slice(i, i + VERIFY_CHUNK))
  }
  verifierAgents += groups.length
  const out = await parallel(groups.map(g => async () => {
    const short = g[0].file.split("/").pop()
    const r = await agent(GROUP_VERIFIER_PROMPT(g), { label: "verify:" + short + "(" + g.length + ")", phase: "Verify", model: "haiku", effort: "low", schema: GROUP_VERDICT_SCHEMA })
    if (!r) return []
    const byIdx = {}
...
// ─── Optimization 3 — run only the angles Scope says apply.
// angle-A is unconditional (a line-by-line scan always has work). If Scope
// returns nothing usable, fall back to the full ladder rather than
// under-reviewing.
const requested = Array.isArray(scope.applicableAngles) ? scope.applicableAngles : []
const picked = CORRECTNESS_ANGLES.filter(a => requested.includes(a.label))
const withA = picked.some(a => a.label === "angle-A")
  ? picked
  : [CORRECTNESS_ANGLES[0]].concat(picked)
const chosenAngles = (picked.length > 0 ? withA : CORRECTNESS_ANGLES).slice(0, P.correctnessAngles)
const skipped = CORRECTNESS_ANGLES.filter(a => !chosenAngles.some(c => c.label === a.label)).map(a => a.label)
if (skipped.length > 0) log("angles skipped as not applicable: " + skipped.join(", "))

const FINDERS = chosenAngles
...
const finderOuts = await parallel(FINDERS.map(f => () =>
  agent(FINDER_PROMPT(f), { label: f.label, phase: "Find", model: "sonnet", schema: CANDIDATES_SCHEMA }).then(r => {
    if (!r) return []
...
// Deliberately full-effort Sonnet — this stage earns its cost.
if (P.sweep) {
  phase("Sweep")
  ...
  const sweep = await agent(
    ...
    { label: "sweep", phase: "Sweep", model: "sonnet", schema: CANDIDATES_SCHEMA }
  )
...
// Low effort — this stage shuffles indices, it does not re-judge code.
phase("Synthesize")
...
const report = await agent(
  ...
  { label: "synthesize", model: "sonnet", effort: "low", schema: REPORT_SCHEMA }
)
